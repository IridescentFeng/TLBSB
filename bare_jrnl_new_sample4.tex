...
\title{Token-Level KL Control for Sequential Safety Alignment: An Analysis and the Limits of Semantic Barriers}
...
{作者 \MakeLowercase{\textit{et al.}}: Token-Level KL Control for Sequential Safety Alignment}
...
\begin{abstract}
安全性与有用性是大型语言模型对齐的两个核心目标。SACPO 等序列化对齐框架将二者解耦为两阶段依次优化，在理论上具有良好的分解性质，但其失效模式尚不清楚。本文对序列化安全对齐给出 token 级分析，揭示两个结构性缺陷：其一，序列级 KL 约束允许优化器将散度集中于个别 token 而以其余位置的零散度摊薄总惩罚，使逐 token 约束在满足全局预算的表象下失效（命题1）；其二，纯安全训练的梯度不显式依赖有用性数据分布，第一阶段习得的有用性能力在第二阶段单调漂移（命题2）。我们进一步证明，逐 token 的 KL 硬屏障可消除摊薄，但其 token-agnostic 性质引入约束强度与可达学习深度之间不可调和的全局权衡（定理1、定理2）。在实证上，我们发现仅将 SACPO 的序列级（DPO）对齐目标替换为 token 级 KL 目标（TDPO），即在安全性与有用性两轴上同时取得显著提升；而纯安全单阶段训练导致有用性灾难性崩塌，直接印证命题2 的漂移分析。受“逐 token 控制”的启发，我们设计并系统检验了一个 token 条件的语义屏障——由 prompt 级门控与 token 级安全探针构成。尽管该屏障在训练中充分 engage（门控在安全 prompt 上开启、探针判别有害 token、其梯度占比随训练上升），它在分布内、对抗 OOD（AdvBench、StrongREJECT）、过度拒绝（XSTest）与安全–有用性 Pareto 四类评测上，相对 token 级 KL 基线均未带来显著改进。诊断表明，屏障作用于表示空间而未改变输出分布。本文据此指出 token 级 KL 控制是序列化安全对齐的关键杠杆，并为表示空间的安全干预提供一个有机制诊断的警示性负面结果。
\end{abstract}


\begin{IEEEkeywords}
大语言模型对齐，基于人类反馈的强化学习，直接偏好优化，安全对齐，token 级 KL 控制，序列化对齐，语义屏障，负面结果。
\end{IEEEkeywords}
...
然而，我们发现SACPO的序列化对齐框架存在两个结构性缺陷。其一，SACPO依赖序列级KL散度约束维持第一阶段能力，但序列级求和允许优化器将全部KL预算集中于语义冲突的关键token，同时令其余token的KL归零来摊薄总惩罚，导致局部约束在看似满足全局预算的情况下实际失效。其二，第二阶段的纯安全训练损失仅在安全数据分布上计算梯度，不显式依赖有用性数据分布，有用性能力差距在训练过程中单调累积而缺乏任何主动抑制机制。

针对上述两个结构性缺陷，本文给出系统的 token 级分析，并据此设计与检验一个自然的干预方案。我们首先证明：对逐 token KL 散度施加直接硬约束虽能消除摊薄自由度，但其 token-agnostic 性质导致约束强度与可达学习深度之间不可调和的全局权衡。受此启发，我们设计 token 条件的语义屏障，由 prompt 级分类器门控与 token 级安全探针（Safety-Aware Probe, SAP）共同决定逐位惩罚强度。在实证上，我们发现：(i) 仅将序列级（DPO）对齐目标替换为 token 级 KL 目标（TDPO），即在安全与有用两轴上同时显著优于 SACPO；(ii) 纯安全单阶段训练导致有用性崩塌，直接印证命题2；(iii) 然而，在已采用 token 级 KL 控制的强基线之上叠加语义屏障，尽管其在训练中机械上充分 engage，在分布内、对抗 OOD、过度拒绝与 Pareto 四类评测上均未带来显著的行为收益——诊断表明它作用于表示空间而不改变输出分布。

本文的主要贡献如下：
\begin{itemize}
    \item 我们对 SACPO 序列化对齐框架进行系统性理论分析，形式化证明了序列级 KL 约束下逐 token 散度的不可辨识性（摊薄）与纯安全训练下有用性能力的无约束漂移两个结构性缺陷。

    \item 我们在实证上确立：token 级 KL 控制（TDPO）在序列化安全对齐中显著优于序列级（SACPO）对齐，并通过纯安全训练下的有用性崩塌直接印证漂移分析。

    \item 我们设计并跨分布内、对抗 OOD、过度拒绝与安全–有用性 Pareto 四类评测系统检验了一个 token 条件的语义屏障，给出一个有机制诊断的负面结果：该屏障机械上充分 engage 却不改变输出行为，作用于表示空间而非输出分布——为表示空间的安全干预提供警示。
\end{itemize}
...
直接偏好优化（DPO）\cite{rafailov2023direct}通过数学重参数化将隐式奖励对应到对数概率比，从而把偏好数据上的强化学习目标转化为一个监督分类问题，彻底绕开显式奖励模型与 PPO。DPO 的简洁性催生了大量变体：身份策略优化（IPO）\cite{azar2024general}引入正则化以抑制极端对数比，序列似然校准（SLiC）\cite{zhao2023slic}以排名损失替代对数似然对比；ORPO\cite{hong2024orpo}将监督微调目标与偏好比值惩罚合并为单阶段目标以减少超参数。上述方法的 KL 约束均作用于整条序列的对数概率，对单个 token 位置的分布漂移缺乏细粒度控制，是本文命题1 所揭示的摊薄问题的共同来源。

\subsection{Token 级偏好学习}

将偏好信号从序列级下放到 token 级是近期对齐研究的重要方向。TDPO\cite{zeng2024token}（ICML 2024）基于 MDP 性能差分引理，将 DPO 的序列级奖励差分解为每步 token 优势函数之和，并引入前向序列级 KL 惩罚以防止各 token 分布的极端偏移；本文以 TDPO1 作为底层优化框架。TIS-DPO\cite{wang2023tisdpo}在离线偏好数据上以 token 级重要性权重重采样，使偏好梯度向关键 token 位置集中，缓解末尾 token 主导（brevity bias）问题。SePO\cite{chen2024sepo}以选择性 token 掩码替代全序列损失，仅在模型预测置信度低于参考策略的位置施加偏好优化梯度，减少对已经对齐 token 的冗余更新。AlignDistil\cite{liu2024aligndistil}将 DPO 的序列级对数比差转化为 token 级分布对齐目标，通过对偏好回答逐位蒸馏实现细粒度的奖励分配。

上述工作表明 token 级信用分配可显著改善偏好学习效率，但它们均聚焦于有用性偏好数据、未系统处理安全性约束。本文实验证明，将序列级对齐目标替换为 token 级 KL 目标（TDPO）在安全对齐场景中同样带来显著收益，并揭示 token 级 KL 控制是序列化安全对齐中真正起作用的杠杆。
...
为降低上述约束优化的实现复杂度，若干工作将拉格朗日框架移植到 DPO 范式：多目标 DPO（MODPO）\cite{zhou2024beyond}保留奖励模型但以线性组合权重平衡多个目标；约束 DPO（C-DPO）\cite{liu2024enhancing}迭代更新拉格朗日乘子同时反复应用 DPO。SafeDPO\cite{kim2025safedpo}将偏好数据限定为纯安全偏好对，以 DPO 形式的单阶段对齐取代复杂的双阶段流程，但牺牲了有用性保持能力；本文实验定量刻画了这一崩塌并将其与命题2 直接对应。基于屏障函数的策略优化（BFPO）\cite{zhang2025bfpo}在连续强化学习场景中引入对数屏障以约束策略偏离，但未针对 token 级分布漂移进行专门设计。原始–对偶 DPO（Primal-Dual DPO）\cite{zhao2024primaldual}将约束偏好学习形式化为鞍点问题，通过自适应拉格朗日乘子梯度实现在线约束满足，但其序列级 KL 约束仍继承摊薄问题。

本文所采用的 SACPO\cite{wachi2024stepwise}（NeurIPS 2024）在理论上证明多约束对齐问题的全局最优策略可被分解为有用性对齐与安全对齐的序列化二阶段过程，但其分解依赖序列级 KL 约束维持各阶段能力。本文系统分析该约束的两个结构性缺陷，并以实验证明仅替换为 token 级 KL 目标即可消除第一个缺陷带来的损失。实验所用数据集包括 PKU-SafeRLHF\cite{ji2024beavertails}，其同时提供有用性与安全性偏好对，分别构成两阶段对齐的训练信号；分布外评测采用 AdvBench\cite{zou2023universal}、StrongREJECT\cite{souly2024strongreject} 与 XSTest\cite{rottger2024xstest}。
...
上述表示层干预方法均以\emph{推理时}操控为手段，无需重新训练。本文所研究的语义屏障则是一种\emph{训练时}的表示空间干预：以探针 $v_\phi$ 对隐状态施加惩罚梯度，试图在训练过程中将安全信号注入模型权重。尽管表示工程类方法已证明推理时干预可改变模型行为，本文的负面结果表明，训练时将安全惩罚作用于表示空间——在实验条件下——并不会改变模型的输出分布，诊断指向探针梯度未能有效传播至生成 logits。这一机制差异为理解表示空间安全干预的边界条件提供了实证参照。
...
\section{Token-Level Analysis and Semantic Barrier}
本节给出序列化安全对齐的 token 级分析，并据此设计本文所检验的干预。我们先证明序列级对齐的两个结构性缺陷：在序列级 KL 预算下，rejected 序列的散度可集中于个别 token 而非分散抑制（命题1，\textit{摊薄}）；以及纯安全训练会使有用性能力相对第一阶段模型单调漂移（命题2，\textit{漂移}）。命题1 表明有效的约束须作用于逐 token 层面——我们证明一个逐 token 的 KL 硬屏障可消除摊薄（定理1），但其约束强度与 token 的语义角色无关，导致约束强度与学习深度之间不可调和的全局权衡（定理2、备注1）。为绕开该权衡，我们设计一个 \textit{token-conditional} 的语义屏障（定义4），由 prompt 级门控 $\sigma(\rho)$ 与 token 级安全探针 $v_\phi$ 相乘构成，并在损失层面刻画其性质：对安全 prompt 有害 token 的选择性压制（定理3），以及良性 prompt 上的梯度门控（定理4）。须强调，以上结果刻画的是屏障\textit{损失}的结构；该结构能否转化为输出分布上的行为收益，是一个经验问题，本文在实验节专门检验。本方法以 TDPO1\cite{zeng2024token} 为底层优化框架。

\subsection{序列级KL约束对逐token散度的不可辨识性}\label{prop1}
...
\textbf{命题1}（序列级KL预算下逐token散度的不可辨识性）。设第二阶段以 $\pi_r$ 为参考模型在 $\mathcal{D}_s$ 上进行偏好优化，以序列级KL预算 $\sum_{t=1}^{T} \mathrm{kl}_t(x, y_r) \le C$ 约束 rejected 序列 $y_r$ 的总散度，其中 $\mathrm{kl}_t(x, y_r) = \mathrm{KL}(\pi_r(\cdot|x, y_{<t}) \,\|\, \pi_\theta(\cdot|x, y_{<t}))$ 为前向逐位 KL 散度。则该约束不蕴含任何逐token的散度上界，对任意时间步 $t^*$ 与任意 $m \in (0, C]$，均存在满足序列级预算的策略 $\pi_\theta^*$，使 $\mathrm{kl}_{t^*}(x, y_r) = m$ 而其余位置 $\mathrm{kl}_t(x, y_r) = 0$。由于 $m$ 可取至 $C$，将散度分散于各位置与集中于单点的策略具有相同的可行状态，序列级约束既不能阻止、也无法识别散度向个别token的集中。证明见附录\ref{app:2}。

命题1直接适用于 TDPO1 的 $\delta$ 项。TDPO 虽以\textit{词元级}对齐为设计动机，但其 KL 惩罚项 $\delta(x, y_c, y_r)$ 仅为逐位 KL 沿序列的求和之差，不约束单个 $\mathrm{kl}_t$。偏好目标因而可被单点过拟合满足，而非通过对有害片段的分布式抑制达成，所得解在 $s_{t^*}$ 之外不再安全。修复需将序列级总量约束替换为逐 token 的独立约束，见\ref{sec:kl-barrier}节。
...
记 $\mathrm{supp}(\mathcal{D}) \triangleq \{x : p_\mathcal{D}(x) > 0\}$ 为数据集 $\mathcal{D}$ 的 prompt 支撑集。

\textbf{假设1}\label{jiashe1}（prompt支撑集不相交，理想化）。安全数据集 $\mathcal{D}_s$ 的 prompt 来自红队攻击类输入，有用性数据集 $\mathcal{D}_h$ 的 prompt 来自良性日常查询，二者在语义空间上高度分离。本文将其理想化地建模为支撑集不相交：
\begin{equation}
    \mathrm{supp}(\mathcal{D}_s) \cap \mathrm{supp}(\mathcal{D}_h) = \emptyset
\end{equation}
此假设针对输入 prompt 分布而非回答内容。真实数据中两类 prompt 的重叠通常很小但非零；记重叠测度为 $\omega$，下文在 $\omega = 0$ 的理想化下展开，$\omega > 0$ 时各结论以 $O(\omega)$ 误差近似成立。
...
\textbf{引理1}\label{gradient1}（损失函数不显式依赖有用性分布）。$\mathcal{L}(\theta; \mathcal{D}_s)$ 为 $\mathcal{D}_s$ 上逐样本损失 $\ell$ 的期望，其梯度为对 $\mathcal{D}_s$ 的 prompt 取期望：
\begin{equation}
...
\subsection{Token 级 Barrier 的设计}\label{token barrier}
Token级KL直接约束可修复命题1，但其 token-agnostic 形式导致约束强度与可达学习深度之间不可调和的全局权衡；语义屏障将其替换为可学习的 token-conditional 结构。

\subsubsection{KL直接约束及其根本局限}\label{sec:kl-barrier}
修复命题1的设计原则是将序列级总量约束替换为 token 级逐位约束，对每个时间步的 KL 值独立施加上界，使任一节点的散度集中无法借由其他节点的零散度获得可行性。对数障碍函数 $-\mu\log(\kappa - \mathrm{kl}_t)$ 兼具光滑性与边界发散性，是约束优化内点法的经典选择\cite{boyd2004convex}。设 $\kappa > 0$ 为 per-token KL 预算上界，对 rejected 序列第 $t$ 个 token 位置的前向逐位 KL 散度 $\mathrm{kl}_t(x, y_r) = \mathrm{KL}(\pi_r(\cdot|x, y_{<t}) \,\|\, \pi_\theta(\cdot|x, y_{<t}))$，定义 token 级 KL 屏障惩罚为：
\begin{equation}
...
\textbf{定理1}（Token级屏障的反摊薄性）。设序列 $y_r$ 的有效长度为 $T_r$，token 级 KL 屏障损失为各位置惩罚之和：
\begin{equation}
...
\textbf{记号（Active Rejected Token）}。Active rejected token 指 rejected 序列 $y_r$ 中 policy 分布低于参考分布的位置：
...
\textbf{命题3}（Active rejected token上的方向冲突）。对任意 $t \in \mathcal{T}_{\text{active}}$，对齐损失与 KL 屏障损失在位置 $t$ 对参数的 token-level 梯度贡献内积为负：
...
\textbf{备注1}（直接约束的token-agnostic性质）。定理2 权衡不可调和的根源在于 $\mathcal{L}^{\text{KL}}_{\text{barrier}}$ 的施加强度仅由 $\text{kl}_t$ 决定，与 token 的语义角色无关，故 $\mu$ 的调节作用于所有 token 之上，无法选择性地保留对语义关键 token 的约束、同时释放对语义中性 token 的约束。突破这一权衡需要 \textit{token-conditional} 的 barrier，其约束强度随 token 的语义特征动态变化。

\textbf{注（屏障选择与局限）}。定理1 的硬 KL 屏障在理论上消除摊薄，但定理2 表明其“约束强度--学习深度”权衡不可调和，根源在于其施加强度仅由 $\text{kl}_t$ 决定、与 token 语义无关（备注1）。因此本文不采用硬 KL 屏障，转而设计 token-conditional 的语义屏障；经过细致调参的硬 KL 屏障能否在实践中取得净收益，留作未来工作。

\subsubsection{语义屏障的构建}\label{sec:semantic-barrier}
本节构造 token-conditional barrier，由两个并行线性探针组成：prompt 级分类器 $g_\phi$ 提供 prompt-level 选择信号，token 级安全探针 $v_\phi$ 提供 token-level 强度信号，二者通过乘法 gating 组合。

\textbf{定义2}（Token级安全探针,SAP）。在策略模型最后一层隐状态 $h_t(x,y) \in \mathbb{R}^d$ 之上挂载线性探针，输出 token 位置 $t$ 的安全评分：
\begin{equation}\label{eq:sap-form}
    v_\phi(h_t) = w_v^\top h_t + b_v \in \mathbb{R}
\end{equation}
参数量仅 $d+1$，不引入显著计算开销。其蒸馏预训练流程见第~\ref{Dmix SAP}节。

\textbf{假设3}（Prompt Embedding的线性可分性）。参考模型对 $\mathrm{supp}(\mathcal{D}_s)$ 与 $\mathrm{supp}(\mathcal{D}_h)$ 的 prompt 生成语义相反的响应（接续有害内容 vs.\ 拒绝回答），prompt 的数据集归属信息因而被编码于参考模型的表示空间。记 $\phi_{\text{ref}}(x) \in \mathbb{R}^d$ 为参考模型在 $x$ 末位 prompt token 处的最后一层隐状态。我们假设该归属信息在 $\phi_{\text{ref}}$ 空间中近似线性可分，这与表示探测文献的实证规律一致\cite{alain2017understanding}。

\textbf{定义3}（Reference-Conditioned Prompt Classifier）。基于假设3，定义 prompt 级判别信号 $\rho$ 为参考模型 prompt embedding 上的线性分类器：
\begin{equation}\label{eq:prompt-classifier}
\rho \triangleq g_\phi(\phi_{\text{ref}}(x)) = w_g^\top \phi_{\text{ref}}(x) + b_g
\end{equation}
$g_\phi$ 通过对带数据集标签的 prompt（$x \in \mathrm{supp}(\mathcal{D}_s)$ 标为正类、$x \in \mathrm{supp}(\mathcal{D}_h)$ 标为负类）的监督二分类训练得到，训练完成后冻结（训练细节见附录\ref{app:refdetect}）。区别于直接使用 reference log-probability margin 的朴素信号，$g_\phi$ 以监督方式直接优化数据集归属判别，不受 completion 序列长度等混杂因素干扰。

\textbf{引理3}（Prompt Classifier的判别能力）。在假设3下，监督训练的 $g_\phi$ 以高概率判别 prompt 所属数据集：
...
\end{equation}
其中 $\delta_1, \delta_2$ 为分类器在两类 prompt 上的经验错误率。Section V 表明 $g_\phi$ 在 Anthropic HH 上达到 $\text{AUC} > 0.95$（$\delta_1, \delta_2 < 0.05$），远高于朴素信号的 $\approx 0.60$。判别能力分析见附录\ref{app:refdetect}。

由引理3，$\sigma(\rho)$ 构成软化的 prompt-level 门控：$x \in \mathrm{supp}(\mathcal{D}_s)$ 时 $\sigma(\rho) \to 1$，$x \in \mathrm{supp}(\mathcal{D}_h)$ 时 $\sigma(\rho) \to 0$。

\textbf{定义4}（Semantic Barrier with Soft Gating）。结合 prompt 级门控 $\sigma(\rho)$ 与 token 级探针 $v_\phi$，语义障碍定义为二者的乘法组合：
\begin{equation}\label{eq:semantic-barrier}
\mathcal{L}^{\text{sem}}_{\text{barrier}} = \underbrace{\sigma(\rho)}_{\text{prompt-level gate}} \cdot \underbrace{\frac{1}{T_r}\sum_{t \in y_r} -\log\sigma\!\left(v_\phi(h_t)\right)}_{\text{token-level barrier}}
\end{equation}
其中 $v_\phi$ 通过蒸馏预训练获得（训练流程见第~\ref{Dmix SAP}节），其输出对不安全 token 为负值、对安全 token 为正值，与上式对数障碍的惩罚方向一致。乘法 gating 使 barrier 受两级判别调制：$\sigma(\rho) \to 0$ 时整个 barrier 归零，与 $v_\phi$ 无关；$\sigma(\rho) \to 1$ 时强度由 $v_\phi$ 逐位决定。

\subsubsection{语义屏障的理论保证}\label{sec:barrier-theory}
\textbf{定理3}（条件性选择压制）。设 SAP 完美拟合定义4的条件目标，即 $v_\phi(h_t) = \tilde{v}(\rho)$。则在 rejected token 上语义障碍的逐 token 惩罚值为：
...
\textbf{备注2}（语义障碍的设计意图）。语义障碍对 rejected 序列每个有害 token 独立施加惩罚，在损失层面旨在迫使对有害片段的分布式抑制，使单点过拟合无法再借由其余位置的零惩罚满足偏好目标。与定理1 的硬屏障不同，其逐位强度由 token 级探针 $v_\phi$ 条件性决定，而非对 $\mathrm{kl}_t$ 施加发散的硬上界；式(\ref{eq:select-ratio})的选择性压制比趋于无穷源于分母趋零（$\mathrm{supp}(\mathcal{D}_h)$ 上 barrier 随 $\sigma(\rho)\to0$ 消失），而非分子无界增长。须强调：这描述的是屏障损失的结构性设计意图；该设计是否在训练后转化为输出分布上对摊薄的实际抑制，不由本节的损失级刻画所保证，而是实验节检验的经验问题。
...
与 $v_\phi$ 的具体取值无关；惩罚的方向排序（不安全 token 惩罚高于安全 token）由蒸馏目标的符号约定保证，即令$c(x,y_r)<0$ 对应不安全回答，与定义4的对数障碍方向一致。

\textbf{定理4}（Benign Prompts上无方向冲突）。对 $x \in \mathrm{supp}(\mathcal{D}_h)$，由引理3以概率 $\geq 1-\delta_2$ 有 $\rho \leq 0$。由定义4，$\mathcal{L}^{\text{sem}}_{\text{barrier}}$ 与不依赖 $\theta$ 的 $\sigma(\rho)$ 成正比，故 barrier 梯度被 $\sigma(\rho)$ 线性压低：
\begin{equation}
...
\end{equation}
当分类器对该 prompt 自信判别（$\rho \ll 0$，$\sigma(\rho) \to 0$）时，barrier 梯度及其与对齐梯度的内积渐近为零：
...
\end{equation}
该结论仅依赖引理3与定义4的乘法 gating 结构，不需 SAP 完美拟合假设。

\textbf{备注4}（语义障碍不构成额外的漂移来源）。命题3 与定理2 表明硬 KL 屏障与对齐梯度的冲突是 token-agnostic 的，在良性 prompt $\mathrm{supp}(\mathcal{D}_h)$ 上同样发生，而该处对齐梯度正是有用性学习信号，冲突即阻碍有用性学习。定理4 表明语义障碍的这一冲突被 $\sigma(\rho)$ 门控限制在 $\mathrm{supp}(\mathcal{D}_s)$ 内，$\mathrm{supp}(\mathcal{D}_h)$ 上其梯度渐近为零。在理论层面，这仅排除了语义屏障自身成为额外漂移来源的可能；它并不蕴含屏障能修复命题2 的漂移。漂移是否被缓解、以及屏障在行为上是否带来安全收益，均为经验问题，见实验节。

\textbf{附加理论性质}。语义障碍的参数梯度被$\mu\sup_t\|\nabla_\theta v_\phi(h_t)\|$全局有界(命题4,证明见附录\ref{app:bdgrad}),不含 KL 屏障在 $\mathrm{kl}_t \to \kappa$时的梯度发散因子。

\subsection{安全探针的离线蒸馏训练}\label{Dmix SAP}
安全探针 $v_\phi$ 与 prompt 级门控 $g_\phi$ 均在第二阶段开始前离线训练并冻结。其监督需要安全与良性两类 prompt 的覆盖，为此我们使用混合数据集 $\mathcal{D}_{\text{mix}} = \mathcal{D}_s \cup \mathcal{D}_h$ 仅作为探针/门控的训练数据；第二阶段的对齐损失仍仅在 $\mathcal{D}_s$ 上计算（见第~\ref{sec:total-loss}~节），$\mathcal{D}_{\text{mix}}$ 不参与对齐目标，亦不作为命题2 漂移的修复手段。

混合数据集为 SAP 蒸馏预训练提供了必要的双类 prompt 覆盖：$v_\phi$须同时见到 $\mathrm{supp}(\mathcal{D}_s)$ 与$\mathrm{supp}(\mathcal{D}_h)$ 的样本，才能学到安全与有用性语义之间的边界。本文以外部安全评估模型 $\mathcal{M}_{\text{safe}}$ 为教师，在 $\mathcal{D}_{\text{mix}}$ 上最小化蒸馏损失：
...
\begin{aligned}
\mathcal{L}_{\text{TLBSB}} = -\mathbb{E}_{(x, y_c, y_r) \sim \mathcal{D}_s}
...
\end{aligned}
\end{equation}
完整训练流程见 Algorithm \ref{alg:1} 与 \ref{alg:2}。

\begin{algorithm}[h]
\caption{TLBSB两阶段训练流程}
\label{alg:1}
\begin{algorithmic}[1]
...
\STATE 以 $\pi_{\text{SFT}}$ 为参考模型，在 $\mathcal{D}_h$ 上用 DPO
       训练，得到有用性对齐策略 $\pi_r$
\STATE \textbf{// 预处理：prompt 级分类器}
\STATE 以 $\pi_r$ 的 prompt embedding 监督训练 $g_\phi$，训练完成后冻结
...
\FOR{each mini-batch $(x, y_c, y_r) \sim \mathcal{D}_s$}
    \STATE 执行 Algorithm~\ref{alg:2}，计算 $\mathcal{L}_{\text{TLBSB}}$
    \STATE $\theta \leftarrow \theta - \eta\nabla_\theta\mathcal{L}_{\text{TLBSB}}$
\ENDFOR
\RETURN $\pi_\theta$
\end{algorithmic}
\end{algorithm}
...
\STATE \textbf{// 总损失}
\STATE $\mathcal{L}_{\text{TLBSB}} \leftarrow
       \mathcal{L}_{\text{align}} +
       \mathcal{L}^{\text{sem}}_{\text{barrier}}$
\RETURN $\mathcal{L}_{\text{TLBSB}}$
\end{algorithmic}
\end{algorithm}
\subsection{待检验的经验问题}
前述分析在损失层面刻画了语义屏障：它选择性作用于安全 prompt 的有害 token（定理3），并在良性 prompt 上被门控关闭（定理4）。然而，损失层面的结构不等于行为层面的效果——屏障经由探针作用于隐状态\textit{表示}，其对模型实际生成分布的影响须经实证检验。由此引出本文的核心经验问题：在已采用 token 级 KL 控制（TDPO）的强基线之上叠加该语义屏障，是否在安全性、有用性、或二者的权衡上带来可测且显著的改进？下一节通过分布内、对抗 OOD、过度拒绝与 Pareto 四类评测予以回答。

\section{实验}
本节回答第~\ref{token barrier}~节末提出的核心经验问题。我们的实验围绕三条线索展开：(i)~确立 token 级 KL 控制相对序列级对齐的优势（第~\ref{exp:tdpo}~节）；(ii)~以纯安全单阶段训练的有用性崩塌印证命题2 的漂移分析（第~\ref{exp:drift}~节）；(iii)~在已采用 token 级 KL 控制的强基线之上，系统检验语义屏障是否带来可测的行为收益（第~\ref{exp:barrier}--\ref{exp:diag}~节）。后者构成本文的主要发现，亦是一个有机制诊断支撑的负面结果。
...
\textit{对比方法.} (1)~\emph{SACPO}~\cite{wachi2024stepwise}：序列级（DPO）安全对齐；(2)~\emph{SafeDPO}：纯安全数据上的单阶段对齐，用于检验命题2；(3)~\emph{TDPO}（记为 \texttt{no\_barrier}）：将第二阶段目标替换为 token 级 KL 目标（TDPO1）、不加任何屏障，是本文最关键的强基线；(4)~\emph{TLBSB-V6}：在 TDPO 之上加语义屏障，探针 $v_\phi$ 以参考模型的 token 级 margin 自监督（域无关，作为原则上的主方法）；(5)~\emph{TLBSB-X1}：同样的屏障，但 $v_\phi$ 从外部安全奖励模型（Beaver-Cost）蒸馏（域匹配的强教师变体）。V6 与 X1 仅在探针监督来源上不同，用以检验负面结果是否对探针质量稳健。

\textit{评测协议.} 关键的近距离比较（方法间差异小）采用\emph{成对胜率}而非逐点均值——我们发现逐点 LLM-as-judge 打分高度聚集、无法分辨相近模型。每个 prompt 上两方法的回答由 DeepSeek 裁判成对比较，并对调位置取平均以消除位置偏置，胜负差以 $\{-3,\dots,+3\}$ 计分、报告平均 margin 与胜/平/负比例。安全性的分布外泛化在 AdvBench（$n{=}519$）与 StrongREJECT（$n{=}313$）上以贪心解码评测不安全率；过度拒绝在 XSTest-safe（$n{=}250$）上评测对安全 prompt 的拒答率。需要说明的是，PKU-SafeRLHF 的分布内安全评测已\emph{饱和}（\texttt{no\_barrier} 的无害性逐点分达 $9.25/10$，$77\%$ 样本满分，仅 $6/83$ 失败），不足以分辨相近方法，因此我们以上述更难的分布外评测作为安全性的主要判别工具。

\subsection{Token 级 KL 控制显著优于序列级对齐}\label{exp:tdpo}
表~\ref{tab:tdpo-vs-sacpo} 给出 \texttt{no\_barrier}（即不含任何屏障的纯 TDPO）与 SACPO 的成对比较。仅将第二阶段的序列级（DPO）目标替换为 token 级 KL 目标，即在有用性与安全性两轴上同时取得显著优势（有用性 margin $+0.43$，安全性 margin $+0.57$，均 $p<0.001$）。这一结果定位了序列化安全对齐中真正起作用的杠杆：是 token 级的 KL 控制，而非任何附加的安全机制。它也为后续问题提供了正确的对照基线——任何屏障的价值都应在这一已经很强的 TDPO 基线\emph{之上}衡量。

\begin{table}[t]
\centering
\caption{TDPO（\texttt{no\_barrier}）对 SACPO 的成对胜率（H$\to$S）。Margin 为平均偏好分（正值表示 TDPO 更优），胜/平/负为对应比例。$^{***}p<0.001$。}
...
命题2 断言纯安全训练缺乏对有用性的主动保护、致其单调漂移。表~\ref{tab:safedpo} 提供直接证据：将对齐限制在纯安全数据上的单阶段 SafeDPO，其有用性（Beaver 有用性奖励）从 $\pi_r$ 的 $5.33$ 崩塌至 $-0.64$，而采用 token 级 KL 控制的方法保持在 $6.04$；成对比较中 X1 对 SafeDPO 的有用性胜率 margin 达 $+1.56$（$p<0.001$）。这印证了命题2 的分析，并解释了为何联合/序列方法必须显式保留有用性信号——它同时构成对纯安全基线（如 SafeDPO）所代表的"受限制度"的实证刻画。
...
$\pi_r$（有用性初始） & $5.33$ & — \\
SafeDPO（纯安全单阶段） & $-0.64$ & — \\
TDPO / TLBSB & $6.04$ & $+1.56^{***}$ \\
\bottomrule
\end{tabular}
\end{table}

\subsection{语义屏障的行为消融：核心负面结果}\label{exp:barrier}
我们在 TDPO 强基线之上加入语义屏障（V6 与 X1），在三类更难的评测上比较。表~\ref{tab:barrier-ablation} 显示：无论以不安全率（AdvBench、StrongREJECT）还是过度拒绝率（XSTest-safe）衡量，加入屏障均未带来\emph{显著}改进。X1 在 AdvBench 上的不安全率略低于 TDPO（$3.3\%$ vs.\ $4.6\%$），但差异不显著（Wilcoxon $p{=}0.056$，McNemar $p{=}0.077$）；在 StrongREJECT 上 X1 与 TDPO 持平（$p{=}0.22$），V6 反而更差。过度拒绝率三者均极低且无差异（McNemar $p{=}1.0$）。分布内成对比较同样无差异：\texttt{no\_barrier} 对 TLBSB-X1 在有用性（margin $+0.11$）与安全性（margin $+0.08$）上均不显著，即屏障相对纯 TDPO 没有增益、若有则略损有用性。

关键地，X1 与 V6 在全部八项对比上无显著差异——\emph{负面结果对探针监督来源稳健}：无论探针是自监督的 token 级 margin（V6），还是蒸馏自一个专用安全奖励模型的强信号（X1），叠加屏障都不改变行为。这排除了"屏障无效仅因探针质量不足"的解释。

\begin{table}[t]
\centering
\caption{语义屏障的行为消融（H$\to$S）。AdvBench/StrongREJECT 报告不安全率（越低越好），XSTest-safe 报告对安全 prompt 的拒答率（越低越好）。无害性均值（$/10$）附于括号。所有方法间差异均不显著（见正文统计检验）。}
...
\midrule
TDPO (\texttt{no\_barrier}) & $4.6$ \;($9.55$) & $15.7$ \;($8.46$) & $1.2$ \\
TLBSB-V6（自监督探针） & $5.4$ \;($9.51$) & $18.5$ \;($8.34$) & $0.8$ \\
TLBSB-X1（蒸馏探针） & $3.3$ \;($9.66$) & $16.3$ \;($8.51$) & $1.6$ \\
\bottomrule
\end{tabular}%
}
\end{table}
...
若屏障以微小的安全提升换取有用性损失，仍可能在权衡上有意义。表~\ref{tab:pareto} 否定了这一可能。以有用性（PKU helpful，$n{=}129$）对汇合不安全率（AdvBench$+$StrongREJECT，$n{=}833$）作图：X1 相对 TDPO 损失有用性（$5.33$ vs.\ $5.57$）以换取可忽略的安全变化（不安全率 $8.3\%$ vs.\ $8.9\%$），方向上得不偿失；V6 则被 TDPO \emph{严格支配}——既更不有用（$5.50$）又更不安全（$10.3\%$）。换言之，屏障没有把前沿向外推，反而落在 TDPO 之内或之上。
...
\midrule
TDPO (\texttt{no\_barrier}) & $5.57$ & $8.9$ \\
TLBSB-X1（蒸馏探针） & $5.33$ & $8.3$ \\
TLBSB-V6（自监督探针） & $5.50$ & $10.3$ \\
\bottomrule
\end{tabular}
\end{table}

\subsection{机制诊断：engage 但不改变输出分布}\label{exp:diag}
负面结果并非源于屏障"未被激活"。表~\ref{tab:diag} 汇总第二阶段训练的内部指标：prompt 级门控在安全 prompt 上 $\sigma(\rho)$ 稳定于 $0.93$--$0.99$（正确开启），token 级探针对有害 token 的输出从 $-0.4$ 降至 $-0.8$（判别性增强），屏障梯度占总梯度的比例从约 $1\%$ 升至约 $15\%$（实质参与优化），且梯度裁剪触发率为 $0\%$。所有指标都表明屏障在\emph{机械层面}充分 engage。

这构成本文负面结果的核心刻画：屏障经由探针作用于隐状态\emph{表示}，其梯度确实改变了参数，却没有相应地改变模型的输出分布——机制层面的 engage 与行为层面的效果之间发生了\emph{解离}。这一现象提示，基于探针/表示方向的安全干预，即便在训练信号上清晰可见，也未必转化为生成行为上的安全收益；这对依赖表示空间操控（probe/representation steering）的安全方法是一个值得注意的警示。

\begin{table}[t]
\centering
\caption{第二阶段训练的机制诊断（TLBSB-X1，取自训练日志）。所有指标表明屏障被充分激活。}
\label{tab:diag}
\begin{tabular}{lc}
\toprule
诊断指标 & 训练过程（起 $\to$ 终） \\
\midrule
门控 $\sigma(\rho)$（安全 prompt） & $0.93 \to 0.99$ \\
探针 $v_\phi$（有害 token） & $-0.4 \to -0.8$ \\
...
\begin{proof}
固定时间步 $t^*$ 与目标值 $m \in (0, C]$。我们在逐token散度配置的层面构造策略 $\pi_\theta^*$，使其散度配置为 $\mathrm{kl}_{t^*}(x, y_r) = m$ 且 $\mathrm{kl}_t(x, y_r) = 0$（$\forall t \neq t^*$）。
...
\end{equation}
对 DPO，$\ell = -\log\sigma(u)$；对 TDPO，$\ell = -\log\sigma(u - \delta)$；无论何种仅依赖 $\mathcal{D}_s$ 的偏好损失，其梯度均为对 $\mathcal{D}_s$ 的 prompt 取期望。由假设1 的理想化 $\mathrm{supp}(\mathcal{D}_s) \cap \mathrm{supp}(\mathcal{D}_h) = \emptyset$，上式不存在对 $x^h \in \mathrm{supp}(\mathcal{D}_h)$ 的积分项，故梯度方向中不含驱动 $\mathcal{H}(\pi_{\theta})$ 减小的显式分量。若两支撑集存在重叠测度 $\omega > 0$，则该显式分量的范数以 $O(\omega)$ 为界，结论以同阶误差近似成立。
\end{proof}
...
首先给出 $\nabla_\theta \mathcal{H}(\theta_k)$ 的显式形式。由 Definition 1 及前向 SeqKL 的定义，$\pi_r$ 不依赖 $\theta$，对 $\mathcal{H}$ 关于 $\theta$ 求梯度：
\begin{equation}
\begin{aligned}
\nabla_\theta \mathcal{H}(\theta_k) 
...
对所有有效节点 $\mathrm{kl}_t < \kappa$，故 $\kappa - \mathrm{kl}_t \in (0, \kappa]$，从而每个 token 的惩罚被有限常数下界：
\begin{equation}
...
在联合优化目标$\min_\theta\,\mathcal{L}_{\text{align}} + \mathcal{L}_{\text{barrier}}$下，barrier对参数$\theta$的梯度为：
\begin{equation}
...
\textbf{设计说明（求和与均值聚合）}。定理1的结论对求和聚合$\sum_t b_t$与均值聚合$\frac{1}{T_r}\sum_t b_t$同样成立：对固定的有效长度$T_r$，均值形式下$t^*$处的惩罚为$\frac{1}{T_r}b_{t^*}$，随$\mathrm{kl}_{t^*} \to \kappa$同样发散至$+\infty$。两种聚合的区别仅在于均值形式将每个token的惩罚整体缩放$1/T_r$，等价于对屏障强度系数$\mu$的重标定，不改变发散性质。本文token级语义屏障（定义4）采用均值形式，以使屏障强度与序列长度解耦。

\subsubsection*{A.6 偏导规约与梯度方向的符号性质}\label{app:active-partial}
给出引理2中两个偏导的完整推导。Active rejected token的定义见\label{eq:active-def}：$t \in \mathcal{T}_{\text{active}} \iff \pi_\theta(y_t|y_{<t}) < \pi_{\text{ref}}(y_t|y_{<t})$。
...
由链式法则（在barrier工作前提$\text{kl}_t < \kappa$下）：
\begin{equation}
...
\end{equation}
其中不等式 $|\gamma_t| < \pi_{\text{ref}}(y_t)$ 是精确界，等价于 $\pi_{\text{ref}}(y_t) < 1$，在 vocabulary $|\mathcal{V}| > 1$ 下恒成立。由此，$\pi_{\text{ref}}(y_t) \leq \beta/\alpha$ 是 $\beta + \alpha\gamma_t > 0$ 的充分条件。实验设置 $\beta=0.1$、$\alpha=0.5$ 给出阈值 $\beta/\alpha = 0.2$，而对齐训练中单 token 的参考概率通常远小于该阈值，故该充分条件在典型训练区间下普遍满足，此时 $\partial \mathcal{L}_{\text{align}}/\partial \log\pi_\theta(y_t) > 0$。

\textbf{与正文公式的关系}。在 $\gamma_t = 0$（即忽略 $\delta_2$ 的 token 级偏导贡献）的简化下，式(\ref{eq:dL-align-full})退化为：
\begin{equation}
...
\begin{proof}
命题3的核心是在每个active rejected token位置上，对齐损失与barrier损失对参数的token-level梯度贡献内积为负。

\textbf{token-level梯度贡献的形式化}。对损失$\mathcal{L} \in \{\mathcal{L}_{\text{align}}, \mathcal{L}^{\text{KL}}_{\text{barrier}}\}$，位置$t$对$\mathcal{L}$关于$\theta$的总梯度的贡献项定义为：
...
\textbf{说明}。命题3的内积是\textit{token-level梯度贡献}的内积，不是完整的$\langle \nabla_\theta \mathcal{L}_{\text{align}}, \nabla_\theta \mathcal{L}^{\text{KL}}_{\text{barrier}} \rangle$。后者的展开包含off-diagonal项$\langle \nabla_\theta \log\pi_\theta(y_t), \nabla_\theta \log\pi_\theta(y_{t'}) \rangle$ for $t \neq t'$，这些项在LLM参数共享下符号未定。命题3的per-token陈述避免了这一不确定性，且足以支撑定理2的全局结论（详见附录\ref{app:trade}）。
...
\end{equation}
两梯度共线，方向相反。这是命题3的token-level方向冲突在全局最优点的体现。
...
\textbf{说明}。证明的关键步骤是Step 3，其严格依赖于命题3，FOC（式(\ref{eq:foc})）是命题3的token-level方向冲突在最优点的全局体现，使$\nabla_\theta \mathcal{L}_{\text{align}}$与$\nabla_\theta \mathcal{B}$严格反向，从而$d\mathcal{L}_{\text{align}}/d\mu$的二次型表达自然为正。Step 4的一阶近似在典型对齐训练区间下成立（margin分布方差远小于其均值）。

\subsubsection*{A.9 Prompt分类器的训练与判别能力分析}\label{app:refdetect}

本节给出定义3的 prompt 级分类器 $g_\phi$ 的训练流程，分析其判别能力作为引理3的依据，并说明为何采用监督分类器而非 reference log-probability margin 朴素信号。

\textbf{$g_\phi$ 的训练}。$g_\phi$ 是 $\phi_{\text{ref}}(x)$ 上的线性 logistic 分类器。训练在第二阶段之前离线完成。以 $\pi_r$ 对 $\mathcal{D}_s \cup \mathcal{D}_h$ 中每个 prompt 作一次前向，取末位 prompt token 的最后一层隐状态得 $\phi_{\text{ref}}(x)$；$x \in \mathrm{supp}(\mathcal{D}_s)$ 标为正类、$x \in \mathrm{supp}(\mathcal{D}_h)$ 标为负类，记标签 $z \in \{+1, -1\}$。参数 $(w_g, b_g)$ 由 logistic 损失拟合：
\begin{equation}
...
\textbf{判别能力分析}。引理3 断言 $g_\phi$ 以高概率判别 prompt 所属数据集，其依据是假设3 的近似线性可分性。假设3 的合理性来自 $\pi_r$ 的行为差异。$\pi_r$ 为有用性对齐策略，对 $\mathrm{supp}(\mathcal{D}_s)$ 的红队 prompt 倾向拒答或回避，对 $\mathrm{supp}(\mathcal{D}_h)$ 的良性 prompt 倾向正常作答；模型须在 prompt 读入完毕时即已"判定"将采取何种行为，故 prompt 的数据集归属信息必然被编码于 prompt 末位的表示中，而末层隐状态正是该决策相关信息最集中之处。表示探测文献普遍观察到此类高层语义属性在深层表示中近似线性可解码\cite{alain2017understanding}，假设3 即这一规律在安全与有用性二分上的实例。

在近似线性可分的前提下，logistic 分类器收敛到一个分离超平面，其在两类 prompt 上的错判仅来自落在超平面错误一侧的少数样本。引理3 中的 $\delta_1, \delta_2$ 即 $g_\phi$ 在两类 prompt 上的经验错误率，而非松弛的理论上界，由假设3 的线性可分程度与分类器训练质量共同决定，并直接由留出 prompt 上的分类性能测得。Section V 表明在 Anthropic HH 上 $g_\phi$ 达到 $\text{AUC} > 0.95$，对应 $\delta_1, \delta_2 < 0.05$。

\textbf{与朴素信号的对比}。一个无需训练的替代方案是直接以 reference log-probability margin 作为 prompt 级判别信号，即由 $\pi_r$ 对 chosen 与 rejected 回答的对数似然之差定出 $\rho$。该朴素信号判别能力薄弱，根源在于它由 completion 而非 prompt 决定。对数似然之差受 chosen 与 rejected 序列长度、token 频率与表述方式等因素强烈干扰，同类 prompt 的 margin 因其回答对写法不同而大幅波动；且 $\pi_r$ 不含安全概念，其对安全与不安全回答的似然偏好与 prompt 是否来自红队并无可靠对应。

$g_\phi$ 从两方面规避这一问题。其一，输入为 prompt 嵌入 $\phi_{\text{ref}}(x)$ 而非 completion，不受回答序列的混杂因素影响；其二，以监督方式直接优化数据集归属判别，学到的正是门控所需的边界，而非依赖某种附带相关性。实证上 $g_\phi$ 在 Anthropic HH 上的 AUC $> 0.95$，而 reference margin 朴素信号仅约 $0.60$，接近随机判别的 $0.50$。这一差距对 TLBSB 是关键的。定理3 与定理4 的选择性门控保证均建立在引理3 成立、即 $\sigma(\rho)$ 确能分离两类 prompt 之上；朴素信号 AUC 约 $0.60$ 时引理3 实际失效，门控近乎无选择性，而 $g_\phi$ 使引理3 成立，相关理论保证方才适用。

\subsubsection*{A.10 定理3的证明}\label{app:condpressure}
\begin{proof}
分三步，依次给出理想探针下的逐 token 惩罚值、参数梯度系数，以及由二者导出的选择性压制比。

\textbf{第一步（惩罚值）}。设理想探针 $v_\phi(h_t) = \tilde{v}(\rho)$。此时定义4中逐 token 项 $-\log\sigma(v_\phi(h_t)) = \log\sigma(\tilde{v}(\rho))$ 与 $t$ 无关，有效长度归一化的求和等于该公共项，故
\begin{equation}
\mathcal{L}^{\text{sem}}_{\text{barrier}}
...
\end{equation}
每个 token 的贡献以标量系数 $\sigma(\rho)\cdot\mu(1-\sigma(v_\phi(h_t)))$ 缩放 $\nabla_\theta v_\phi(h_t)$。故惩罚值（第一步）与梯度系数均含 $\sigma(\rho)$ 因子。

\textbf{第三步（选择性压制比）}。以惩罚值或逐 token 梯度系数度量某 prompt 上的压制强度，两者均可写作 $\sigma(\rho)\cdot s$ 的形式，其中 $s$ 为有限且严格为正的常数。以惩罚值衡量时 $s = 1.313\mu$ 或 $0.313\mu$；以梯度系数衡量时 $s = \mu(1-\sigma(\tilde{v}(\rho)))$，因 $\tilde{v}(\rho) \in \{-1, +1\}$ 同样有限且非零。由引理3，当 $\delta_1, \delta_2 \to 0$ 时 $\sigma(\rho) \to 1$（$x \in \mathrm{supp}(\mathcal{D}_s)$）、$\sigma(\rho) \to 0$（$x \in \mathrm{supp}(\mathcal{D}_h)$）。因此
\begin{equation}
...
\textbf{命题4}（语义障碍的有界梯度性质）。对任意 prompt $x$ 与 rejected 序列 $y_r$，定义4的语义障碍 $\mathcal{L}^{\text{sem}}_{\text{barrier}}$ 对策略参数 $\theta$ 的梯度满足
...
\end{equation}
该上界与 per-token KL 预算 $\kappa$ 无关，不含任何随状态发散的因子，且对 $v_\phi$ 的任意取值均成立。
...
\textbf{说明}。命题4 与\ref{sec:kl-barrier}节 KL 屏障的对照即语义障碍稳定性的来源。KL log-barrier $-\mu\log(\kappa - \mathrm{kl}_t)$ 的梯度系数为 $\frac{\mu}{\kappa - \mathrm{kl}_t}$，在 $\mathrm{kl}_t \to \kappa$ 时发散；语义障碍以有界因子 $1-\sigma(v_\phi(h_t)) \in (0,1)$ 取代发散因子 $\frac{1}{\kappa - \mathrm{kl}_t}$，从而消除该发散。式(\ref{eq:bdgrad})右端的 $\sup_{t \in y_r}\|\nabla_\theta v_\phi(h_t)\|$ 在标准正则性条件下有限：由 $v_\phi(h_t) = w_v^\top h_t + b_v$ 得 $\nabla_\theta v_\phi(h_t) = (\nabla_\theta h_t)\,w_v$，只要 $\|w_v\|$ 有界且隐状态 $h_t$ 对 $\theta$ 的 Jacobian 有界即可。
