...
def tdpo_loss(chosen_logps_margin: torch.FloatTensor,
              ...
              rejected_barrier_loss: torch.FloatTensor,  # 障碍墙惩罚
              critic_loss: torch.FloatTensor,  # 🚀 新增：哨兵的自监督损失
    ...
    # 🌟 端到端融合：Helpful 梯度 + 软障碍惩罚 + 哨兵自学习梯度 (权重 0.5 防喧宾夺主)
    losses = losses + rejected_barrier_loss + 0.5 * critic_loss
...
class BasicTrainer(object):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str,
        ...
        self.policy = policy
        # 🚀 [入编] 正式为策略模型挂载 Safety Critic 价值头
        if not hasattr(self.policy, 'safety_critic'):
            hidden_size = self.policy.config.hidden_size
            self.policy.safety_critic = nn.Linear(hidden_size, 1).to(self.policy.device)
            # 正态分布初始化，防止初始阶段干扰太大
            self.policy.safety_critic.weight.data.normal_(mean=0.0, std=0.01)
            self.policy.safety_critic.bias.data.zero_()
    ...
    def tdpo_concatenated_forward(self, model: nn.Module, reference_model: nn.Module,
        ...
        # 🚀 保持开启 output_hidden_states，榨干显存，追求极致拟合！
        ...
        all_logps_margin, all_position_kl, all_logps = _tdpo_get_batch_logps(all_logits, reference_all_logits,
        ...
        # ====================================================================
        # 🚀 [BSB-TDPO 算力全开版] End-to-End Soft-Gated Bilevel Optimization
        # ====================================================================
        # 1. 探针：安全教官对指令类别的嗅觉 (Margin)
        ref_chosen_logps = chosen_logps - chosen_logps_margin.detach()
        ref_rejected_logps = rejected_logps - rejected_logps_margin.detach()
        ref_margin = ref_chosen_logps - ref_rejected_logps

        # 使用 Soft-Gating (平滑门控)，让梯度的传递像丝绸一样顺滑，彻底告别抖动！
        is_safety_prompt = torch.sigmoid(ref_margin / 1.0)  # [batch_size]

        # 2. Safety Critic 输出安全评分
        v_safe = model.safety_critic(hidden_states)[:, :-1].squeeze(-1)
        v_safe_rejected = v_safe[half:]

        # 3. 构建 Critic 的绝对真理标签 (Target)
        # 用平滑门控区分：高度危险(-1.0) 还是 仅仅是烂代码(+1.0)
        #target_v = torch.where(ref_margin > 0.0, -1.0, 1.0).unsqueeze(-1).expand_as(v_safe_rejected)
        target_v_rejected = -torch.ones_like(v_safe_rejected)  # rejected = -1.0（不安全）

        labels_rejected = concatenated_batch['concatenated_labels'][half:, 1:].clone()
        loss_mask_rejected = (labels_rejected != -100).float()

        # 4. Critic 的 MSE 拟合损失 (由外部 AdamW 统一优化，完美收敛！)
        #critic_loss_per_token = (v_safe_rejected - target_v) ** 2
        critic_loss_per_token = F.smooth_l1_loss(v_safe_rejected, target_v_rejected, reduction='none')
        critic_loss = (critic_loss_per_token * loss_mask_rejected).sum(-1) / (loss_mask_rejected.sum(-1) + 1e-8)

        # 5. 软障碍硬约束 (Log-Barrier)
        h_barrier = torch.sigmoid(v_safe_rejected)
        mu = 0.1  # 墙体高度
        #barrier_penalty_per_token = -mu * torch.log(h_barrier + 1e-8)
        max_penalty = 5.0  # 这个值可以调，通常 3.0~10.0 之间
        barrier_penalty_per_token = -mu * torch.log(h_barrier + 1e-8)
        barrier_penalty_per_token = torch.clamp(barrier_penalty_per_token, max=max_penalty)

        # 惩罚拦截：仅对被确认为 Safety 的 Prompt 实施硬墙拦截！
        rejected_barrier_loss = (barrier_penalty_per_token * loss_mask_rejected).sum(-1) / (
                    loss_mask_rejected.sum(-1) + 1e-8)
        rejected_barrier_loss = rejected_barrier_loss * is_safety_prompt.detach()
        # ====================================================================

        return chosen_logps_margin, rejected_logps_margin, chosen_position_kl, rejected_position_kl, \
            chosen_logps, rejected_logps, rejected_barrier_loss, critic_loss
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], loss_config: DictConfig, train=True):
        ...
        if loss_config.name == 'tdpo':
            chosen_logps_margin, rejected_logps_margin, chosen_position_kl, rejected_position_kl, policy_chosen_logps, policy_rejected_logps, rejected_barrier_loss, critic_loss \
            ...
            losses, chosen_rewards, rejected_rewards = tdpo_loss(chosen_logps_margin, rejected_logps_margin,
                                                                 ...
                                                                 rejected_barrier_loss, critic_loss,  # 🚀 这里传进去
